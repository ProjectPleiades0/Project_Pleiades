...
resource "aws_eip" "eip" {
  instance = aws_instance.pleiades-instance.id
  domain   = "vpc"
}

resource "aws_ebs_volume" "pleiades-volume" {
  availability_zone = aws_instance.pleiades-instance.availability_zone
  size              = 40

  tags = {
    Name = "pleiades-Volume"
  }
}

resource "aws_volume_attachment" "example_attachment" {
  device_name = "/dev/sdh" # The device name may vary based on the OS
  volume_id   = aws_ebs_volume.pleiades-volume.id
  instance_id = aws_instance.pleiades-instance.id

  # Forces detachment of the volume before destroying, allowing Terraform to manage the volume's lifecycle
  force_detach = true
}

resource "aws_instance" "pleiades-instance" {
