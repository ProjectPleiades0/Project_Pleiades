...
from apps.pleiades_core import _preload_modules
from apps.pleiades_core.containers import Container
from apps.pleiades_core.services import (
    CalculateUserMessageScoreService,
    ProcessUserMessageService,
)
from pleiades_core.plugins.registry import ensure_plugin_registry
...
async def task_runner(
    ...
    plugins = [
        "alcyone-market-analyzer",
        "celestial-chart-searcher",
        "stellar-observer",
        "cosmic-wallet",
        "astral-dex", 
        "pleiades",
...
async def send_plugins(pub: redis.Publisher = Provide["publisher"]):
    ...
    for i, plugin in enumerate(plugins):
        if plugin["slug"] == "stellar-educator":
